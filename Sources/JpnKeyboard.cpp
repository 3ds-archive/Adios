...
namespace CTRPluginFramework {
  ...
  void Komoji(u16 &moji) {
    ...
    std::string const
      str1 = "あいうえおアイウエオわワカつツやゆよヤユヨ",
      str2 = "ぁぃぅぇぉァィゥェォゎヮヵっッゃゅょャュョ";
  ...
  void Dakuten(bool handakuten, u16 &moji) {
    ...
    // 濁点用
    const std::string str1 = "かきくけこさしすせそたちつてとはひふへほカキクケコサシスセソタチツテトハヒフヘホ";
    const std::string str2 = "がぎぐげござじずぜぞだぢづでどばびぶべぼガギグゲゴザジズゼゾダヂヅデドバビブベボ";
    
    // 半濁点用
    const std::string str3 = "はひふへほハヒフヘホ";
    const std::string str4 = "ぱぴぷぺぽぱぴぷぺぽ";
  ...
  void DrawKeyboard(const Screen &scr) {
    ...
    scr.DrawSysfont("戻る", 42, 194);
    if( Controller::IsKeyPressed(B) || (Controller::IsKeyPressed(Touchpad) && TouchRect(28, 191, 68, 22)) )
    ...
    if( Controller::IsKeyPressed(Touchpad) && TouchRect(224, 191, 68, 22) ) {
      KeyboardOpened = false;
    }
    
  }
  
  std::string JpnKeyboard(void) {
    if( !Process::IsPaused() ) {
      Process::Pause();
    }
    ...
    if( !PluginMenu::GetRunningInstance()->WasOpened() ) {
      Process::Play();
    }
