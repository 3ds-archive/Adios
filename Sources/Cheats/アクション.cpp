...
namespace CTRPluginFramework
{
	...
	void Action14( void )
	{
		Process::Write32( 0x33099FE4 + get12c(), 0x066B0600 );
		Process::Write32( 0x33099FB4 + get12c(), 0x00038001 );
		Process::Write32( 0x33099FB0 + get12c(), 0x00000036 );
	}
	...
	void RunAction(MenuEntry *entry)
	{
		// 実行
		...
		if ( entry->Hotkeys[1].IsDown() )
		{
			switch( action_number )
			{
				...
				default:
				{
					Process::Write32(0x33099fe4 + get12c(), 0x060D0600);
	...
	void ActionNumber(MenuEntry *entry)
	{
		action_number = *GetArg<int>(entry);
	}
	...
	{
		ActFolder = new MenuFolder(name);
		*ActFolder += EntryWithHotkey(new MenuEntry( Color(ccNormalOrange) << "アクション実行", RunAction ),
			{
				Hotkey(L + DPadUp, "実行"),
				Hotkey(L + DPadDown, "停止・解除"),
			});
		
		int index;
		for( const std::string &act_name : ActionNames )
		{
			MenuEntry *a = new MenuEntry( 4, Color(ccNormalOrange) << act_name, ActionNumber );
			a->SetArg( new int(index) );
			*ActFolder += a;
			index++;
		}
